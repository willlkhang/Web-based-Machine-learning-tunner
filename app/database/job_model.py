...
class Job(Document):
    ...
    dataset_name = StringField(required=True, choices=["mnist_number", "mnist_fashion"])
    ...
    @classmethod
    def create_or_get_job(cls, epochs, learning_rate, batch_size, dataset_name):
        #check exist data
        job = cls.objects(epochs=epochs, learning_rate=learning_rate, batch_size=batch_size, dataset_name=dataset_name).first() #find and return the first match data of the object from the database
        if job:
